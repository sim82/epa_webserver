...
static void recAssign(tree *tr, int model, size_t entries, size_t width, nodeptr p)
{
  if(isTip(p->number, tr->mxtips))
  ...
  else
    {
      ...
      /* assign xVector, xVector_FLOAT and conditional scaling array */
      ...
      if(tr->useFloat)
	tr->partitionData[model].xVector_FLOAT[p->number - tr->mxtips -1] = (float*)malloc_aligned(sizeof(float) * entries);
      else
